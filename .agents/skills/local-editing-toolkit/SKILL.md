...
description: Integrates private local video-editing utilities with this repository's Stage 0A–3 Vlog workflow. Use when an Agent needs local transcription, audio inspection, deduplication, beat analysis, rendering, captions, motion cards, or editor-timeline export while preserving the approved Stage decisions and validation gates.
---
...
Private purchased tools, music, sound effects, templates, and their original instructions remain outside this repository, normally in the user's Antigravity global skills or another private local directory. Do not copy or commit them into this repository. It is safe to commit this bridge Skill because it contains no private implementation or media asset.

Discover a tool by its capability and available files; do not assume a brand name or a fixed absolute path. Record the exact script/tool path, version or content hash, command parameters, and output path when a tool affects an official result. Check dependencies before use. Do not install packages or download models unless that action is already authorized.
...
Prefer native macOS paths and available commands such as `python3` and `ffmpeg`; discover their actual locations rather than embedding machine-specific paths. Quote paths containing spaces or non-ASCII characters. Keep generated intermediates under `process/workspace/temp/` and formal outputs under the Stage-specific `process/workspace/project-output/` directory.
